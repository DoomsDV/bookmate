...
const requireToken = (token: string | undefined) =>
	requireApiToken(token, createCustomersError, 'No hay sesion valida para restaurar clientes.');
...
export const POST: APIRoute = async ({ locals, params }) => {
	try {
		...
		return toErrorResponse(error, 'No fue posible restaurar el cliente.');
	}
};
