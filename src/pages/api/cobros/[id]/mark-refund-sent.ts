...
import { CobrosApiError, markRefundSentWithOrds } from '../../../../lib/cobros';
...
export const POST: APIRoute = async ({ locals, params }) => {
	try {
		...
		const result = await markRefundSentWithOrds(token, id);
		...
		const err =
			error instanceof CobrosApiError
				? error
				: new CobrosApiError('No fue posible marcar el reembolso como enviado.', 500);
		return Response.json(
