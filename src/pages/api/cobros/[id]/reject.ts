...
import { CobrosApiError, rejectCobroWithOrds } from '../../../../lib/cobros';
...
export const POST: APIRoute = async ({ locals, params, request }) => {
	try {
		...
		const result = await rejectCobroWithOrds(token, id, reason);
		...
		const err =
			error instanceof CobrosApiError
				? error
				: new CobrosApiError('No fue posible rechazar el comprobante.', 500);
		return Response.json(
