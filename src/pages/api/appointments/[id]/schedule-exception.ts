...
import {
	...
	approveScheduleExceptionWithOrds,
...
export const POST: APIRoute = async ({ params, locals }) => {
	try {
		...
		const result = await approveScheduleExceptionWithOrds(token, appointmentId);
		...
		return toErrorResponse(error, 'No fue posible descartar la advertencia.');
	}
};
