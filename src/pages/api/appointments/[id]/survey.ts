...
export const POST: APIRoute = async ({ params, locals }) => {
	try {
		...
		const result = await sendAppointmentSurveyWithOrds(token, appointmentId);
		...
		return toErrorResponse(error, 'No fue posible enviar la encuesta.');
	}
};
