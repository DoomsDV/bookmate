...
import { ChatApiError, getChatMessagesWithOrds } from '../../../../../../lib/chat';
import {
...
export const GET: APIRoute = async ({ locals, params }) => {
	try {
		...
		const messages = await getChatMessagesWithOrds(token, sessionId);
		...
		return toErrorResponse(error, 'No fue posible obtener los mensajes del chat.');
	}
};
