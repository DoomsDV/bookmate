...
const updatePagination = (meta: SpecialtiesListMeta) => {
	...
	const summary = root.querySelector('[data-specialties-pagination-summary]');
	if (summary) {
		summary.innerHTML = `
			Página <strong>${currentPage}</strong> de <strong>${totalPages}</strong>
			<span aria-hidden="true">-</span>
			Total: <strong>${totalRecords}</strong> especialidades
		`;
	}

	const currentLabel = root.querySelector('[data-specialties-pagination-current]');
	if (currentLabel) currentLabel.textContent = String(currentPage);

	const prevBtn = root.querySelector<HTMLButtonElement>('[data-specialties-page-prev]');
	const nextBtn = root.querySelector<HTMLButtonElement>('[data-specialties-page-next]');
	if (prevBtn) {
...
export const initSpecialtiesListControls = () => {
	...
	document.addEventListener('click', (event) => {
		...
		const pageBtn = target.closest<HTMLButtonElement>(
			'[data-specialties-page-prev], [data-specialties-page-next]'
		);
