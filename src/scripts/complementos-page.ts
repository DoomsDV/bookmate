...
const ADDON_COPY: Record<string, string> = {
	ODONTOGRAM_3D:
		'Ficha clínica interactiva y evolución de tratamientos (Pronto con soporte 3D avanzado).',
};
...
const ODONTO_SAMPLE: Record<number, string> = {
	16: 'caries',
	14: 'restoration',
	11: 'crown',
	26: 'restoration',
...
const odontogramPreviewHtml = () => {
	const tooth = (n: number) => {
		const finding = ODONTO_SAMPLE[n];
		const findingClass = finding ? ` is-${finding}` : '';
		return `<span class="complementos-odonto__tooth${findingClass}">${n}</span>`;
	};
	return `
		<div class="complementos-card__preview" aria-hidden="true">
			<div class="complementos-odonto">
				<div class="complementos-odonto__arch">${ODONTO_UPPER.map(tooth).join('')}</div>
				<div class="complementos-odonto__arch">${ODONTO_LOWER.map(tooth).join('')}</div>
			</div>
		</div>
	`;
};
